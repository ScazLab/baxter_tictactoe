#ifndef __CARTESIAN_ESTIMATOR_TTT__
#define __CARTESIAN_ESTIMATOR_TTT__

#include "robot_perception/hsv_detection.h"
#include "robot_perception/cartesian_estimator.h"

class SegmentedObjTTT: public SegmentedObj
{
public:
    ...
    /* CONSTRUCTOR */
    SegmentedObjTTT(std::vector<double> _size, hsvColorRange _col);
    SegmentedObjTTT(std::string _name, std::vector<double> _size, int _area_thres, hsvColorRange _col);
...
/**
 * Class that is able to detect objects from a range of TTT colors that
 * define their color.
 */
class CartesianEstimatorTTT : public CartesianEstimator
{
protected:
     ...
     * @return true/false if success/failure
     */
    bool addObject(std::string _name, double _h, double _w, hsvColorRange _TTT);
    ...
    bool addObjects(std::vector<std::string> _names, cv::Mat _o,
                    std::vector<hsvColorRange> _TTTs);
...
public:
    /* CONSTRUCTORS */
    CartesianEstimatorTTT(std::string  _name);
};
