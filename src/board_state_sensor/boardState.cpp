...
BoardState::BoardState(string name, bool _show) : ROSThreadImage(name), doShow(_show), r(40) // 40Hz
{
    ...
    // ROS_ASSERT_MSG(_n.getParam("baxter_tictactoe/hsv_red",hsv_red_symbols),
    //                                                       "No HSV params for RED!");
    // hsv_red=hsvColorRange(hsv_red_symbols);

    // XmlRpc::XmlRpcValue hsv_blue_symbols;
    // ROS_ASSERT_MSG(_n.getParam("baxter_tictactoe/hsv_blue",hsv_blue_symbols),
    //                                                        "No HSV params for BLUE!");
    // hsv_blue=hsvColorRange(hsv_blue_symbols);

    // ROS_ASSERT_MSG(_n.getParam("baxter_tictactoe/area_threshold",area_threshold),
    //                                                                 "No area threshold!");
...
void BoardState::InternalThreadEntry()
{
    while(ros::ok())
    {
        ROS_INFO("InternalThreadEntry BS");
        if (!_img_empty)
        {
            board.resetState();
            cv::Mat img_copy;
            ...
            ROS_INFO("InternalThreadEntry BS. Res: %i", res);
